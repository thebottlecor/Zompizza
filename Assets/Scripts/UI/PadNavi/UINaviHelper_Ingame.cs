...
public class UINaviHelper_Ingame : MonoBehaviour
{
    ...
    public UINavi Villager_Reconnection()
    {
        var needsObj = UIManager.Instance.villagerUI.giveBtn.gameObject;

        if (needsObj.activeSelf)
        {
            return villager_first[2];
        }
        else
        {
            return villager_first[0];
        }
    ...
    public UINavi Shop_Orders_Reconnection()
    {
        ...
        if (shops.midnightPanel.activeSelf)
        {
            shops_close.left = shops_orders_midnight[0];
            shops_close.right = shops_orders_midnight[0];

            return shops_orders_midnight[0];
        }
